...
# ─────────────────────────────────────────────────────────────────────────────
# FASE 3: COMPARATIVA DE JUGADORES LADO A LADO
# ─────────────────────────────────────────────────────────────────────────────

def render_comparativa_jugadores(d2, key_prefix="comp"):
    """Comparativa de 2 jugadores en columnas paralelas con barras.

    d2: dict {jugador: stats_dict} de la fuente seleccionada.
    Para cada metrica muestra una fila con barra enfrentada; el mejor
    valor de cada fila se pinta en verde y el otro en gris.
    """
    if not d2 or len(d2) < 2:
        return

    st.markdown("---")
    st.subheader("⚖️ Comparativa de Jugadores")

    jugadores = list(d2.keys())
    col_a, col_b = st.columns(2)
    with col_a:
        jug_a = st.selectbox("Jugador A", jugadores, index=0,
                              key=f"{key_prefix}_jug_a",
                              format_func=lambda x: str(x).title())
    with col_b:
        idx_b = 1 if len(jugadores) > 1 else 0
        jug_b = st.selectbox("Jugador B", jugadores, index=idx_b,
                              key=f"{key_prefix}_jug_b",
                              format_func=lambda x: str(x).title())
    ...
    stats_a = d2.get(jug_a, {})
    stats_b = d2.get(jug_b, {})

    # Metricas: (etiqueta, sinonimos, direccion)
    #   direccion: "max" = mayor mejor, "min" = menor mejor, None = neutral
    metricas = [
        ("⭐ Puntuación Global",
         ["puntiación global", "puntuación global", "puntacion global",
          "puntiación", "puntuación", "puntacion"], "max"),
        ("🎯 Media 180 por partida",
         ["media 180 por partida", "180 por partida", "180 por partido",
          "media 180", "media de 180"], "max"),
        ("📊 Promedio puntos total",
         ["promedio puntos total", "promedio puntos", "media puntos",
          "puntos total", "promedio dardos", "average"], "max"),
        ("🎮 Legs por partido",
         ["legs totales", "total legs", "legs por partido",
          "leg por partido"], None),
        ("✅ Promedio Checkouts",
         ["promedio checkouts", "promedio checkout", "checkouts",
          "checkout"], "max"),
        ("🏆 Número victorias",
         ["número victorias", "numero victorias", "n. victorias",
          "n victorias", "victorias"], "max"),
        ("❌ Número derrotas",
         ["número derrotas", "numero derrotas", "n. derrotas",
          "n derrotas", "derrotas"], "min"),
        ("📈 Porcentaje victoria",
         ["porcentaje victoria", "porcentaje de victoria", "% victoria",
          "%victoria", "porc. victoria"], "max"),
        ("🎲 Índice Volatilidad",
         ["índice volatilidad", "indice volatilidad",
          "índice de volatilidad", "volatilidad"], "min"),
    ]

    def _buscar(stats_dict, sinonimos, excluidas):
        for syn in sinonimos:
            s = syn.lower()
            for k, v in stats_dict.items():
                if k in excluidas:
    ...
    filas_html.append(
        ...
        f"<div style='flex:1;text-align:left;font-weight:800;font-size:16px;"
        f"color:#1e293b;'>{str(jug_a).title()}</div>"
        f"<div style='width:170px;text-align:center;font-size:12px;"
        f"color:#94a3b8;font-weight:700;'>VS</div>"
        f"<div style='flex:1;text-align:right;font-weight:800;font-size:16px;"
        f"color:#1e293b;'>{str(jug_b).title()}</div>"
        f"</div>"
    )

    usadas_a, usadas_b = set(), set()
    for etiqueta, sinonimos, direccion in metricas:
        ka, va = _buscar(stats_a, sinonimos, usadas_a)
        kb, vb = _buscar(stats_b, sinonimos, usadas_b)
        if ka is None and kb is None:
            continue
        if ka: usadas_a.add(ka)
        if kb: usadas_b.add(kb)

        na, nb = _num(va), _num(vb)

        # Determinar mejor
        mejor_a = mejor_b = False
        if direccion and na is not None and nb is not None and na != nb:
            if direccion == "max":
                mejor_a, mejor_b = na > nb, nb > na
            else:
                mejor_a, mejor_b = na < nb, nb < na

        # Barras proporcionales al maximo de los dos
        maxv = max(abs(na or 0), abs(nb or 0))
        pa = int(100 * abs(na or 0) / maxv) if maxv > 0 else 0
        pb = int(100 * abs(nb or 0) / maxv) if maxv > 0 else 0

        color_a = VERDE if mejor_a else GRIS
        color_b = VERDE if mejor_b else GRIS
        peso_a = "800" if mejor_a else "600"
        peso_b = "800" if mejor_b else "600"

        txt_a = str(va) if va is not None else "—"
        txt_b = str(vb) if vb is not None else "—"

        filas_html.append(
            f"<div style='display:flex;align-items:center;gap:10px;"
            f"margin-bottom:9px;'>"
            # Valor A
            f"<div style='width:55px;text-align:right;font-weight:{peso_a};"
            f"font-size:14px;color:#1e293b;'>{txt_a}</div>"
            # Barra A (crece hacia la izquierda)
            f"<div style='flex:1;height:14px;background:#f1f5f9;"
            f"border-radius:7px;overflow:hidden;display:flex;"
            f"justify-content:flex-end;'>"
            f"<div style='width:{pa}%;background:{color_a};height:100%;"
            f"border-radius:7px;'></div></div>"
            # Etiqueta central
            f"<div style='width:170px;text-align:center;font-size:11.5px;"
            f"color:#64748b;font-weight:600;'>{etiqueta}</div>"
            # Barra B (crece hacia la derecha)
            f"<div style='flex:1;height:14px;background:#f1f5f9;"
            f"border-radius:7px;overflow:hidden;'>"
            f"<div style='width:{pb}%;background:{color_b};height:100%;"
            f"border-radius:7px;'></div></div>"
            # Valor B
            f"<div style='width:55px;text-align:left;font-weight:{peso_b};"
            f"font-size:14px;color:#1e293b;'>{txt_b}</div>"
            f"</div>"
        )
    ...
    compacto = " ".join(
        line.strip() for line in contenedor.splitlines() if line.strip()
    )
    st.markdown(compacto, unsafe_allow_html=True)
    st.caption("💡 Verde = mejor valor en esa métrica. "
               "Legs por partido no se compara (neutral).")
...
def render_comparativa_jugadores(d2, key_prefix="comp"):
    """Comparativa de 2 jugadores lado a lado con barras enfrentadas.

    d2: dict {jugador: {stat: valor}} de la fuente seleccionada.
    Para cada metrica muestra una fila con el valor y una barra proporcional
    a cada lado; el mejor de la fila va en verde.
    """
    if not d2 or len(d2) < 2:
        st.info("ℹ️ Se necesitan al menos 2 jugadores con datos para comparar.")
        return
    ...
    with col1:
        j1 = st.selectbox("Jugador 1", jugadores, index=0,
                          key=f"{key_prefix}_j1",
                          format_func=lambda x: str(x).title())
    with col2:
        j2 = st.selectbox("Jugador 2", jugadores,
                          index=1 if len(jugadores) > 1 else 0,
                          key=f"{key_prefix}_j2",
                          format_func=lambda x: str(x).title())

    if j1 == j2:
        st.warning("⚠️ Elige dos jugadores distintos para comparar.")
        return

    stats1 = d2.get(j1, {})
    stats2 = d2.get(j2, {})

    # (etiqueta, sinonimos, direccion)  direccion: 1 mayor=mejor,
    # -1 menor=mejor, 0 neutral (sin ganador)
    metricas = [
        ("⭐ Puntuación Global",
         ["puntiación global", "puntuación global", "puntacion global",
          "puntiación", "puntuación", "puntacion"], 1),
        ("🎯 Media 180 por partida",
         ["media 180 por partida", "180 por partida", "180 por partido",
          "media 180", "media de 180"], 1),
        ("📊 Promedio puntos total",
         ["promedio puntos total", "promedio puntos", "media puntos",
          "puntos total", "promedio dardos", "average"], 1),
        ("🎮 Legs por partido",
         ["legs totales", "total legs", "legs por partido",
          "leg por partido"], 0),
        ("✅ Promedio Checkouts",
        ...
        ("🏆 Número victorias",
         ["número victorias", "numero victorias", "n. victorias",
          "n victorias", "victorias"], 1),
        ("❌ Número derrotas",
         ["número derrotas", "numero derrotas", "n. derrotas",
          "n derrotas", "derrotas"], -1),
        ("📈 Porcentaje victoria",
         ["porcentaje victoria", "porcentaje de victoria", "% victoria",
          "%victoria", "porc. victoria"], 1),
        ("🎲 Índice Volatilidad",
         ["índice volatilidad", "indice volatilidad",
          "índice de volatilidad", "volatilidad"], -1),
    ]

    def _buscar(stats_dict, sinonimos, usadas):
        for syn in sinonimos:
            syn_l = syn.lower()
            for k, v in stats_dict.items():
                if k in usadas:
                    continue
                if syn_l in str(k).lower().strip():
                    return k, v
        return None, None
    ...
    header = (
        ...
        f"gap:12px;align-items:center;margin:14px 0 10px;'>"
        f"<div style='text-align:right;font-size:18px;font-weight:800;"
        f"color:#1e3a8a;'>{str(j1).title()}</div>"
        f"<div style='font-size:13px;color:#94a3b8;font-weight:700;'>VS</div>"
        f"<div style='text-align:left;font-size:18px;font-weight:800;"
        f"color:#7c2d12;'>{str(j2).title()}</div>"
        f"</div>"
    )

    filas = []
    usadas1, usadas2 = set(), set()
    VERDE = "#22c55e"
    GRIS = "#cbd5e1"
    for etiqueta, sinonimos, direccion in metricas:
        k1, v1 = _buscar(stats1, sinonimos, usadas1)
        k2, v2 = _buscar(stats2, sinonimos, usadas2)
        if k1 is None and k2 is None:
            continue
        if k1: usadas1.add(k1)
        if k2: usadas2.add(k2)

        n1, n2 = _num(v1), _num(v2)
        maxv = max([abs(x) for x in (n1, n2) if x is not None] or [0])

        def _barra(n, lado, color):
            if n is None or maxv == 0:
                pct = 0
            else:
                pct = max(4, round(100 * abs(n) / maxv))
            just = "flex-end" if lado == "izq" else "flex-start"
            return (
                f"<div style='display:flex;justify-content:{just};'>"
                f"<div style='width:{pct}%;height:14px;border-radius:7px;"
                f"background:{color};'></div></div>"
            )

        # Decidir colores (verde el mejor)
        c1 = c2 = GRIS
        if direccion != 0 and n1 is not None and n2 is not None and n1 != n2:
            if (direccion == 1 and n1 > n2) or (direccion == -1 and n1 < n2):
                c1 = VERDE
            else:
                c2 = VERDE
        elif direccion != 0 and n1 is not None and n2 is None:
            c1 = VERDE
        elif direccion != 0 and n2 is not None and n1 is None:
            c2 = VERDE

        t1 = str(v1) if v1 is not None else "—"
        t2 = str(v2) if v2 is not None else "—"
        peso1 = "800" if c1 == VERDE else "500"
        peso2 = "800" if c2 == VERDE else "500"

        filas.append(
            f"<div style='display:grid;"
            f"grid-template-columns:52px 1fr 190px 1fr 52px;gap:8px;"
            f"align-items:center;padding:7px 0;"
            f"border-bottom:1px solid #f1f5f9;'>"
            f"<div style='text-align:right;font-weight:{peso1};"
            f"font-size:14px;'>{t1}</div>"
            f"{_barra(n1, 'izq', c1)}"
            f"<div style='text-align:center;font-size:12px;color:#64748b;"
            f"font-weight:600;'>{etiqueta}</div>"
            f"{_barra(n2, 'der', c2)}"
            f"<div style='text-align:left;font-weight:{peso2};"
            f"font-size:14px;'>{t2}</div>"
            f"</div>"
        )

    if not filas:
        st.info("ℹ️ No hay métricas comparables entre estos dos jugadores.")
        return
    ...
    st.markdown(html_compacto, unsafe_allow_html=True)
    st.caption("💡 La barra verde marca al mejor en cada métrica. "
               "En ❌ derrotas y 🎲 volatilidad gana el valor más bajo. "
               "🎮 Legs por partido es informativo (sin ganador).")
